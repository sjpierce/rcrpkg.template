---
title: "Example Script for Rendering PDF Output"
author: 
...
csl: apa.csl                               # Citation style formatting rules  
params:
  SourceDir: "scripts/"                    # Relative path to folder holding this file
  SourceFile: "Example_Render_to_PDF.qmd"  # Name of this script file
  LogFile: "Example_Render_to_PDF.pdf"     # Default name of rendered output file
format: 
  pdf:                                       # Settings for pdf output
    output-file: "Example_Render_to_PDF.pdf" # Default output file name
    output-ext: "pdf"
    documentclass: scrartcl                  # LaTeX document type (KOMA-Script)
    papersize: letter
    geometry:                                # Specify page margins
      - top=1in
      - bottom=1in
      - left=1in
      - right=1in
    number-sections: true                       # Whether to auto-number each section
    number-depth: 3                             # No. of layers in section numbers
    toc: true                                   # Whether to include TOC
    toc-depth: 3                                # No. of layers in TOC
    colorlinks: true                            # Whether to color URLs
    template-partials: 
      - Latex_Partials/title.tex
    include-in-header: 
      - file: Latex_Partials/compact-title.tex
      - text: | 
          \usepackage{fancyhdr}
...
build in hidden headings and text that are useful while editing the script but 
will be omitted from the knitted PDF output file. 
...
before section headings helps keep the figures and tables in their respective 
sections. 
\end{comment}
...
library(tidyverse)    # loads dplyr, forcats, ggplot2, lubridate, purrr, readr, 
                      # stringr, tibble, and tidyr all at once. 
                      # for arrange(), filter(), group_by(), mutate(), 
...
# Heading Level 1
Put text here. Here's an example of citing a paper so you can see how the 
reference list gets populated: @Bryan-RN3900 is a very useful overview of why 
you should use version control tools. The citation data about the that paper is 
in the file identified by the `bibliography:` key in the YAML header. The file
identified by the `csl:` key in the YAML header controls how in-text citations 
and reference list entries are formatted. 
