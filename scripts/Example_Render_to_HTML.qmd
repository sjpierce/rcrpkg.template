...
params:
  ...
  SourceFile: "Example_Render_to_HTML.qmd"  # Name of this script file
  LogFile: "Example_Render_to_HTML.html"    # Default name of rendered output file
...
format: 
  html:                               # Settings for HTML output
    output-file: "Example_Render_to_HTML.html"  # Default output file name
...
This is useful because it tells us which commit in the Git history we would need
to be using to make sure we are running the exact same code. Sometimes another
person is not using the most current code, or has changed the code in some way
