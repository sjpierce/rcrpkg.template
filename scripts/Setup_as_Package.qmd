...
files created by certain chunks below. It isn't necessary to save the output of 
this script. 
...
projects. There are examples of other Quarto (`*.qmd` files) scripts in this 
repository. 
:::
...
library(devtools)   # for document(), session_info()
library(fs)         # for dir_create(), file_move()
library(here)       # for here(), i_am()
library(usethis)    # for use_description(), use_gpl_license(), use_news_md(),
                    # use_package_doc(), use_proprietary_license(), 
                    # use_readme_rmd()
